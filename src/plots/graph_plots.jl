...
function plot_graph_3d(
    ...
    gr();
    plt = plot3d(;
        layout=1, grid=false, size=size, dpi=300, zlim=(-0.1, 0.3), ticklabelsize=10
    )
    ## plot edges
    for e in edges(Γ.G)
        orig = src(e);
...
function plot_function_3d(
    ...
    gr();
    plt = plot3d(;
        layout=1, size=size, dpi=300, topmargin=-20mm, xticks=false, yticks=false
    );
    if grid_off == true
