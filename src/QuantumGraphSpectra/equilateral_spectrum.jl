...
function eigvals_quantum(Γ::EquilateralMetricGraph; K=3, sorted=true, only_vertex=false)
    ...
    n=nv(Γ.G); m=ne(Γ.G)
    if is_bipartite(Γ.G) 
        no_vertex = n-2; no_non_vertex_even = m-n+2; no_non_vertex_odd = m-n+2
    else
        no_vertex = n-1; no_non_vertex_even = m-n+2; no_non_vertex_odd = m-n
    end
...
function eigfunc_norm(λ::Number, A::Vector, B::Vector, ℓ::Number, m::Int)
    ...
    for j = 1:m
        Ae = A[j]; Be = B[j]
        norm += (2*λ_sqrt*ℓ*(Ae^2+Be^2)+(Ae^2-Be^2)*sin(2*λ_sqrt*ℓ)+2*Ae*Be*(1-cos(2*λ_sqrt*ℓ)))/(4*λ_sqrt)
    end
    return sqrt(norm)
end
...
function eigfunc_norm_vertex(Γ::EquilateralMetricGraph, λ::Number, μ::Number, Φ::Vector)
    ...
    for (j,e) in enumerate(edges(Γ.G))
        ...
        norm += 1/(4*λ_sqrt) * (Φ[v1]^2 - (1/sin(acos(1-μ))*(Φ[v2]-Φ[v1]*(1-μ)))^2)*sin(2*acos(1-μ))
        norm += 1/(4*λ_sqrt) * (2*Φ[v1]*(1/sin(acos(1-μ))*(Φ[v2]-Φ[v1]*(1-μ)))*(1-cos(2*acos(1-μ))) )
    end
    return sqrt(norm)
end
...
function eigen_quantum(Γ::EquilateralMetricGraph; K=3, sparse_svd=false, round_off=false)
    ...
    if K%2 == 0
        total_no_evals = Int(1+K*no_vertex_ev+(K/2)*no_non_vertex_ev_odd+(K/2-1)*no_non_vertex_ev_even)
    else
        total_no_evals = Int(1+K*no_vertex_ev+((K-1)/2)*no_non_vertex_ev_odd+((K-1)/2)*no_non_vertex_ev_even)
    end

    # assemble output matrices
    Λ = zeros(total_no_evals); A = spzeros(m,total_no_evals); B = spzeros(m,total_no_evals)
    ...
    # k even 
        if k%2 == 0
            if k == 0
                norm = (sqrt(m*Γ.ℓ))
                Λ[eval_count] = 0; A[:,eval_count] = ones(m)/norm
                # next eval
                eval_count+=1
            else
        ### non-vertex k even ##################################
                ...
                if sparse_svd == false
                    ...
                    _,V = eigs(Symmetric(sqrtD⁻¹*L̃*sqrtD⁻¹-μ*I), sigma=10^(-7), nev=no_non_vertex_ev_even)
                end
                for i = 1:no_non_vertex_ev_even
                    # extended eigenvectors
                    Φ_ext = sqrtD⁻¹*(V[:,end-i+1])
                    # store coefficients
                    A_q = zeros(m); B_q = zeros(m)
                    for (j,e) in enumerate(edges(Γ.G))
                        e_0 = src(e); e_ℓ = dst(e) 
                        w1 = n+k*(j-1)+1
                        A_q[j] = Φ_ext[e_0]
                    ...
                    Λ[eval_count] = λ; A[:,eval_count] = A_q./norm; B[:,eval_count] = B_q./norm
                    # next eval
                    eval_count += 1
                end
            end
        ### vertex k even ######################################
            for i = 2:no_vertex_ev+1
                # quantum graph eigenvalue
                λ = (1/Γ.ℓ*(acos(1-Mu_vec[i])+k*pi))^2; Φ = Φ_vec[:,i]
                # store coefficients
                A_q = zeros(m); B_q = zeros(m)
                for (j,e) in enumerate(edges(Γ.G))
                    e_0 = src(e); e_ℓ = dst(e) 
    ...
    ### k odd ##############################################################################
        ...
        ### vertex k odd #######################################
            for i=no_vertex_ev+1:-1:2
                # quantum graph eigenvalue
                λ=(1/Γ.ℓ*(-acos(1-Mu_vec[i])+(k+1)*pi))^2; Φ=Φ_vec[:,i]
                # store coefficients
                A_q=zeros(m); B_q=zeros(m)
                for (j,e) in enumerate(edges(Γ.G))
                    e_0=src(e); e_ℓ=dst(e) 
...
function count_eigvals_K(Γ::EquilateralMetricGraph, K::Int)
    ...
    # k_even
    if K%2 == 0
        return Int(1 + K*no_vertex_ev + K/2*no_non_vertex_ev_odd + (K/2-1)*no_non_vertex_ev_even)
    else 
        return Int(1 + K*no_vertex_ev + (K-1)/2*no_non_vertex_ev_even + ((K+1)/2-1)*no_non_vertex_ev_odd)
    end
end