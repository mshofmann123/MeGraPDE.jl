...
function test_problem_2_4_4(;eigfunc_no=5)
    ...
    A_q = σ.A[:,eigfunc_no]; B_q = σ.B[:,eigfunc_no]; λ_q = σ.Λ[eigfunc_no]
    ...
    for j=1:ne(Γ.G)
        ...
        push!(u_deriv,(t,x) -> exp(-t*λ_q)*(-A_q[j]*sqrt(λ_q)*sin(sqrt(λ_q)*x)+B_q[j]*sqrt(λ_q)*cos(sqrt(λ_q)*x)));
    end
    # return
