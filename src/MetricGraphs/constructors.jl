...
function metric_tree_graph(; ℓ=1)
    ...
    # second level
    G_tree = add_children(G_tree,3,2); G_tree = add_children(G_tree,4,2);
    # third level
    G_tree = add_children(G_tree,5,2); G_tree = add_children(G_tree,6,2); G_tree = add_children(G_tree,7,2); G_tree = add_children(G_tree,8,2);
    #
    c_0 = 0.6;
    a = c_0/2;
    h_1 = sqrt(ℓ-c_0^2/4);
...
function metric_cycle_graph(;ℓ=1)
    G = CycleGraph(6);  
    hoehe = ℓ/2*sqrt(3);
    coords = [[0,2*ℓ],[h,2*ℓ-hoehe],[hoehe,2*ℓ-2*hoehe],[0,0],[-hoehe,hoehe],[-hoehe,2*hoehe]];
...
function metric_graphene_graph(; ℓ=1)
    G = CycleGraph(6);  
    add_vertex!(G); add_edge!(G,1,7)
    for i = 8:12
        add_vertex!(G); add_edge!(G,i-1,i)
    end
    add_edge!(G,12,7)

    h = ℓ/2*sqrt(3);
    coords = [[0,2*ℓ],[h,2*ℓ-h],[h,2*ℓ-2*h],[0,0],[-h,h],[-h,2*h],[0,3*ℓ],[-h,3*ℓ+h],[-h,3*ℓ+2*h],[0,3*ℓ+3*h],[h,3*ℓ+2*h],[h,3*ℓ+h]];
...
function metric_diamond_graph(; ℓ=1)
    G = cycle_graph(4); add_edge!(G,1,3)
    hoehe = ℓ/2*sqrt(3)
...
function metric_lollipop_graph(;n1=3, n2=2, ℓ=1)
    ...
    elseif n1==3 && n2==1
        ...
        error("metric_lollipop_graph is currently implemented for lollipop_graph(3,2) and lollipop_graph(3,1) only")
    end
end
